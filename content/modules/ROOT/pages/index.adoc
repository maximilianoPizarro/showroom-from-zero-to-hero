= Red Hat Developer Hub: De Cero a Producción
...
Tu usuario es `user\{n}` (donde `\{n}` es tu número asignado). La contraseña es `Welcome123!`.

* **Developer Hub**: https://backstage-developer-hub-developer-hub.{cluster-domain}
* **Gitea**: https://gitea.{cluster-domain}
* **ArgoCD**: https://openshift-gitops-server-openshift-gitops.{cluster-domain}
* **DevSpaces**: https://devspaces.{cluster-domain}

NOTE: Reemplaza `{cluster-domain}` con el dominio de tu cluster.
