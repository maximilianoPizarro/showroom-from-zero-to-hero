...
In Gitea (`https://gitea-gitea.__CLUSTER_DOMAIN__`), confirm two new repositories under your user:
...
----
https://neuralbank-frontend-%USER_NAME%-neuralbank.__CLUSTER_DOMAIN__
----
...
oc get secret -n %USER_NAME%-neuralbank \
...
|===
| | Scaffolded Apps (your `%USER_NAME%-neuralbank`) | Pre-deployed `neuralbank-stack`
...
| **Frontend URL**
| `https://neuralbank-frontend-%USER_NAME%-neuralbank.__CLUSTER_DOMAIN__`
| `https://neuralbank.__CLUSTER_DOMAIN__`

| **Credentials**
| API Key from Kubernetes Secret
| `%USER_NAME%` / `Welcome123!` via Keycloak
|===

NOTE: To add a full interactive OIDC flow to scaffolded apps, you would need to create an OIDC client in Keycloak's `neuralbank` realm, configure the redirect URI, and implement PKCE/Authorization Code flow in the frontend. The pre-deployed `neuralbank-stack` demonstrates this pattern.

== Summary

You added **frontend** and **MCP** with templates, verified GitOps and the catalog, and understood how the reverse proxy routes all traffic through Kuadrant gateways with **API Key authentication**. The pre-deployed `neuralbank-stack` shows the OIDC alternative.
