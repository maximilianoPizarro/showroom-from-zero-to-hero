...
. En **Developer Hub**, ve a **Create** y elige la plantilla **Neuralbank: Frontend** (nombre puede variar ligeramente).
. Completa el formulario con un nombre coherente, por ejemplo `neuralbank-frontend`.
. Asigna el **owner** a tu usuario de taller (`{user_name}`, mismo criterio que en el backend).
...
. Repite el flujo **Create** con la plantilla **Neuralbank: Customer Service MCP** (o título equivalente).
. Usa un nombre claro, por ejemplo `customer-service-mcp`.
. Mantén el **owner** alineado con tu usuario para facilitar el filtrado en el catálogo.
...
. Abre **Argo CD**.
. Localiza **Applications** para frontend y MCP (nombres ligados al repo o al namespace del workshop).
. Verifica **sync** y **health**; sincroniza manualmente si tu rol lo permite y el estado lo requiere.
...
. Filtra por tu **owner** o busca por nombre.
...
. En la **OpenShift Console**, selecciona el namespace del MCP o el indicado por el instructor.
. Navega a **Networking** y busca recursos de **Gateway** y **HTTPRoute** (la ubicación exacta depende de la versión y de los CRD instalados).
