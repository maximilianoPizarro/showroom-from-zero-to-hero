...
. Log in to Developer Hub at `https://backstage-developer-hub-developer-hub.__CLUSTER_DOMAIN__`
...
. Open Gitea: `https://gitea-gitea.__CLUSTER_DOMAIN__`
...
. Open ArgoCD: `https://openshift-gitops-server-openshift-gitops.__CLUSTER_DOMAIN__`
    ...
    rules:
      authentication:
        ...
        oidc:                  # Priority 1 — fallback
          jwt:
            issuerUrl: https://rhbk.__CLUSTER_DOMAIN__/realms/neuralbank
          credentials:
            authorizationHeader:
              prefix: Bearer
          priority: 1
...
curl -s -H "X-API-Key: $API_KEY" \
  "https://neuralbank-backend-%USER_NAME%-neuralbank.__CLUSTER_DOMAIN__/api/customers" \
  | python3 -m json.tool

# List credits
curl -s -H "X-API-Key: $API_KEY" \
  "https://neuralbank-backend-%USER_NAME%-neuralbank.__CLUSTER_DOMAIN__/api/credits" \
  | python3 -m json.tool
----
