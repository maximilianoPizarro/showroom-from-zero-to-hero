...
* Acceso a Developer Hub con tu usuario (formato `user\{n}`) y contraseña `Welcome123!`.
* Permisos para crear componentes desde plantillas en el catálogo del workshop.

NOTE: Sustituye `\{n}` por tu número de usuario asignado en todos los pasos donde aparezca `userN`.
...
* **Propietario (owner)**: tu entidad de catálogo, por ejemplo `user1`, `user2`, … según tu usuario del workshop (`userN`).

[.console-input]
----
name = neuralbank-backend
owner = userN    # usa tu número real, p. ej. user3
----
