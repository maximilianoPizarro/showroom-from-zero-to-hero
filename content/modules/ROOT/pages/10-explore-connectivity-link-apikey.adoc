...
NFL Wallet is deployed in the `nfl-wallet-prod` namespace. It uses the same Quarkus backend image but is protected by **API Key** instead of OIDC, making it ideal for **machine-to-machine (M2M)** access.
...
== Test: Request With a Valid API Key (expect 200)
...
All 10 should return `200`. If you extend the loop beyond 120 requests within one minute, you will see `429 Too Many Requests`.
...
The NFL Wallet API includes an interactive Swagger UI:

----
https://nfl-wallet.apps.cluster-l9nhj.dynamic.redhatworkshops.io/q/swagger-ui
----

NOTE: Swagger UI requests also pass through the AuthPolicy. The API Key header must be included.
