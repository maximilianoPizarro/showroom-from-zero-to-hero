...
. Navega a **Argo CD**: `https://openshift-gitops-server-openshift-gitops.{cluster_domain}`
. En la lista de **Applications**, filtra por tu namespace `{user_name}-neuralbank`.
...
. Haz clic en el nombre para abrir la vista de **Tree** o **Network**.
. Observa **Deployments**, **Services**, **Routes**, **ConfigMaps**, **Secrets** (referenciados, no valores), y en el caso MCP los recursos de **Gateway API** y políticas.
...
. Abre **Developer Hub** y entra en la ficha de **neuralbank-backend**.
. Busca la sección de **dependencies** o diagrama de relaciones (según plugins instalados).
. Identifica vínculos hacia **APIs**, **systems** u otros **components** (frontend, MCP).
...
. En el catálogo, abre la entidad **API** asociada al backend (nombre acorde al dominio Neuralbank).
. Revisa la definición **OpenAPI** incrustada o enlazada (según cómo se publicó en `catalog-info.yaml`).
...
NOTE: Si no ves políticas, revisa que los CRD de Kuadrant estén instalados en el cluster. Todos los recursos deberían estar en `{user_name}-neuralbank`.
