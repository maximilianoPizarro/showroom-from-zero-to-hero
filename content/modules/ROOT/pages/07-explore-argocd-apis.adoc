...
. Open **Developer Hub**: `https://backstage-developer-hub-developer-hub.__CLUSTER_DOMAIN__`
...
. Go to ArgoCD: `https://openshift-gitops-server-openshift-gitops.__CLUSTER_DOMAIN__`
...
spec:
  generators:
    - git:
        repoURL: https://gitea-gitea.__CLUSTER_DOMAIN__
        revision: HEAD
        directories:
  ...
  template:
    ...
    spec:
      project: default
      source:
        repoURL: "https://gitea-gitea.__CLUSTER_DOMAIN__/{{path[0]}}/neuralbank-backend.git"
        targetRevision: HEAD
        path: manifests
      destination:
