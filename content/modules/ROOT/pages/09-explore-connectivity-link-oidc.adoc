...
. Open the frontend: `https://neuralbank.__CLUSTER_DOMAIN__`
  ...
  auth:
    tokenSource:
      ...
      cookie:
        name: jwt
  provider:
    authorizationEndpoint: https://rhbk.__CLUSTER_DOMAIN__/realms/neuralbank/protocol/openid-connect/auth
    clientID: neuralbank-frontend
    issuerURL: https://rhbk.__CLUSTER_DOMAIN__/realms/neuralbank
    redirectURI: https://neuralbank.__CLUSTER_DOMAIN__/auth/callback
    tokenEndpoint: https://rhbk.__CLUSTER_DOMAIN__/realms/neuralbank/protocol/openid-connect/token
  targetRef:
...
sequenceDiagram
    participant User
    participant Frontend as neuralbank.__CLUSTER_DOMAIN__
    participant Gateway as Istio Gateway
    participant OIDC as OIDCPolicy
    participant Keycloak
    participant Backend

    User->>Frontend: Open https://neuralbank.__CLUSTER_DOMAIN__
...
curl -s -o /dev/null -w "HTTP Status: %{http_code}\n" \
  https://neuralbank.__CLUSTER_DOMAIN__/api/v1/customers
----
...
----
TOKEN=$(curl -s -X POST \
  "https://rhbk.__CLUSTER_DOMAIN__/realms/neuralbank/protocol/openid-connect/token" \
...
* Users access the frontend at `https://neuralbank.__CLUSTER_DOMAIN__`
* The `OIDCPolicy` CR redirects to Keycloak for authentication
* Credentials: `%USER_NAME%` / `Welcome123!` (same as Developer Hub)
